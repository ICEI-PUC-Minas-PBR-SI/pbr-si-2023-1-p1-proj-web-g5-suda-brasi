...
btn_Entrar.addEventListener('click', (e) => {
    ...
    for (let i = 0; i < buscadb.dados.length; i++) {
        if ((email_Logar.value === buscadb.dados[i].email) && (senha_Logar.value === buscadb.dados[i].senha)) {
            loginValido = true;
            sessionStorage.setItem("usuario", JSON.stringify(buscadb.dados[i]))
        }
        ...
        if ((senha_Logar.value != buscadb.dados[i].senha) && (email_Logar.value === buscadb.dados[i].email)) {
            senha_Logar.classList.add("animation");
        }
    }
